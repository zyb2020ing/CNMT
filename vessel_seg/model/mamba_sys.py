...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size, in_chans, dims, depths, d_state, drop_rate, attn_drop_rate, 
                 drop_path_rate, norm_layer, patch_norm, use_checkpoint):
        super().__init__()
...
class VSSMDecoder(nn.Module):
    def __init__(self, dims, depths, d_state, drop_rate, attn_drop_rate, drop_path_rate, 
                 norm_layer, use_checkpoint, final_upsample):
        super().__init__()
        self.num_layers = len(depths)
        self.embed_dim = dims[0]
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        ...
        if self.final_upsample == "expand_first":
            ...
            self.output = nn.Conv2d(in_channels=self.embed_dim, out_channels=4, kernel_size=1, bias=False)
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
                 ...
                 norm_layer=nn.LayerNorm, patch_norm=True, use_checkpoint=False, final_upsample="expand_first", **kwargs):
        super().__init__()
        self.encoder = VSSMEncoder(
            patch_size=patch_size, in_chans=in_chans, dims=dims, depths=depths, d_state=d_state, 
            drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, drop_path_rate=drop_path_rate, 
            norm_layer=norm_layer, patch_norm=patch_norm, use_checkpoint=use_checkpoint
        )
        self.decoder = VSSMDecoder(
            dims=dims, depths=depths, d_state=d_state, drop_rate=drop_rate, 
            attn_drop_rate=attn_drop_rate, drop_path_rate=drop_path_rate, 
            norm_layer=norm_layer, use_checkpoint=use_checkpoint, final_upsample=final_upsample
        )
